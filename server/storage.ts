...
export class DatabaseStorage implements IStorage {
  ...
  async getEvents(filters?: { status?: string; upcoming?: boolean }): Promise<Event[]> {
    ...
    if (filters?.upcoming) {
      // Show events as upcoming if their start time hasn't passed yet (Moscow timezone UTC+3)
      const nowMoscow = new Date(new Date().toLocaleString('en-US', { timeZone: 'Europe/Moscow' }));
      // Convert current Moscow time to UTC for database comparison
      const nowMoscowUTC = new Date(nowMoscow.getTime() - (3 * 60 * 60 * 1000));
      conditions.push(gte(events.startsAt, nowMoscowUTC));
    }
